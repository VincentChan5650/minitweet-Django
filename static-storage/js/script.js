...
$(document).ready(function(){
  ...
  function attachTweet(tweetValue, prepend, retweet){
    ...
    "<div class=\"panel\" id=\"mediabody\">"
        +tweetContent + "</div>\
    <br>" + "via @\
    <span class='bg-info text-center'>" +
    "<a href='"+ tweetUser.url +"'>" + tweetUser.username +"</a>"+
    " | "+ dateDisplay +
     "</span>\
     <a href='/tweet/"+ tweetValue.id +"' class='btn btn-primary btn-xs'>View</a>"+
      " " + timesince + " | <a href='' class='tweet-like'>Like</a> \
      <a href='tweet/" + tweetValue.id +"/delete' class='btn btn-danger btn-xs'>Delete</a> \
     </div><hr />"
  ...
  $("#tweet-form").submit(function(event){
    ...
    if (charsCurrent >=0){
      ...
      $('#tweet-form').html("<p>Tweet too long, cannot send<a href='/' >Retry</a><p>")
    }
  })
});
